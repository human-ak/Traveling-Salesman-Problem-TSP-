...
def create_cities_dataframe(cities=None):
    ...
    "Udaipur", "Goa", "Kochi", "Amritsar", "Mysore",
    "Hyderabad", "Kolkata", "Jodhpur", "Shimla", "Darjeeling",
    "Chennai", "Aurangabad", "Rishikesh", "Pondicherry", "Hampi"
        ]
...
def main(count, df=None):
    ...
    for i in range(count):  # run the simulation x times
        ...
        print(f"Running similation: {i}:  Found total distance: {distance} km")
        # append the distance to the distance list
        distance_list.append(distance)
...
def cities_cli(cities, count):
    ...
    Example: ./tsa_cities.py cities "New York" "Knoxville" --count 2
    """
...
def simulate(count):
    ...
    Example:
        ./tsa_cities.py simulate --count 15
